...
class Tuple(object):
    ...
    def add_new_flow(self, column_values):
        ...
        if self.debug > 2:
            ...
            print('Adding flow: Start:{}, Dur:{:.10f},{},{},{},{},{},{},{},{},{},{},{},{},{}\n'.format(column_values['starttime'].strftime("%Y-%m-%dT%H:%M:%S.%f"),column_values['dur'],column_values['proto'],column_values['saddr'],column_values['sport'],'<->',column_values['daddr'],column_values['dport'],column_values['state'],0,0,column_values['pkts'],column_values['bytes'],column_values['sbytes'],''))
        ...
        # Get the size
        try:
            self.current_size = float(column_values['bytes'])
        except ValueError:
        ...
        # Get the duration
        try:
            self.current_duration = float(column_values['dur'])
        except ValueError:
            # It can happen that we dont have this value in the binetflow
            self.current_duration = 0.0
        # Get the protocol
        self.proto = str(column_values['proto'])
...
class Processor(multiprocessing.Process):
    ...
    def get_columns(self, line):
        ...
        Valid formats are:
            ...
            - Future work: add the persers from the configuration file
        """
        column_values = {}
        column_values['starttime'] = False
        column_values['endtime'] = False
        column_values['dur'] = False
        column_values['proto'] = False
        column_values['appproto'] = False
        column_values['saddr'] = False
        column_values['sport'] = False
        column_values['dir'] = False
        column_values['daddr'] = False
        column_values['dport'] = False
        column_values['state'] = False
        column_values['pkts'] = False
        column_values['spkts'] = False
        column_values['dpkts'] = False
        column_values['bytes'] = False
        column_values['sbytes'] = False
        column_values['dbytes'] = False
        try:
            # Heuristic detection: if we have more ',', then is CSV; if we have more '{', then is JSON.
            amount_commas = line.count(',')
            amount_brackets = line.count('{')
            try:
                # Supossedly not happening in the flows, only the suricata alerts
                if '1970-01-00' in line:
                    line = line.replace('1970-01-00','1970-01-01')
                if '1900-01-00' in line:
                    line = line.replace('1900-01-00','1900-01-01')
            ...
            if data_type == 'json':
                ...
                if data['event_type'] != 'flow':
                    return False
                #if self.debug > 4:
                    #print('JSON data detected: {}'.format(line))
                # JSON
                column_values['starttime'] = datetime.strptime(data['flow']['start'].split('+')[0], '%Y-%m-%dT%H:%M:%S.%f') # We do not process timezones now
                column_values['endtime'] = datetime.strptime(data['flow']['end'].split('+')[0], '%Y-%m-%dT%H:%M:%S.%f')  # We do not process timezones now
                difference = column_values['endtime'] - column_values['starttime']
                column_values['dur'] = difference.total_seconds()
                column_values['proto'] = data['proto']
                try:
                    column_values['appproto'] = data['app_proto']
                except KeyError:
                    pass
                column_values['saddr'] = data['src_ip']
                try:
                    column_values['sport'] = data['src_port']
                except KeyError:
                    # Some protocols like icmp dont have ports
                    column_values['sport'] = '0'
                # leave dir as default
                column_values['daddr'] = data['dest_ip']
                try:
                    column_values['dport'] = data['dest_port']
                except KeyError:
                    # Some protocols like icmp dont have ports
                    column_values['dport'] = '0'
                column_values['state'] = data['flow']['state']
                column_values['pkts'] = int(data['flow']['pkts_toserver']) + int(data['flow']['pkts_toclient'])
                column_values['spkts'] = int(data['flow']['pkts_toserver']) 
                column_values['dpkts'] = int(data['flow']['pkts_toclient']) 
                column_values['bytes'] = int(data['flow']['bytes_toserver']) + int(data['flow']['bytes_toclient'])
                column_values['sbytes'] = int(data['flow']['bytes_toserver'])
                column_values['dbytes'] = int(data['flow']['bytes_toclient'])
                # Print the netflows back
                # StartTime,Dur,Proto,SrcAddr,Sport,Dir,DstAddr,Dport,State,sTos,dTos,TotPkts,TotBytes,SrcBytes,Label
                #f=open('output','a')
                #f.write('{},{},{},{},{},{},{},{},{},{},{},{},{},{},{}\n'.format(column_values['starttime'],column_values['dur'],column_values['proto'],column_values['saddr'],column_values['sport'],'<->',column_values['daddr'],column_values['dport'],column_values['state'],0,0,column_values['pkts'],column_values['bytes'],column_values['sbytes'],''))
                #f.flush()
                #f.close()
            elif data_type == 'csv':
                #if self.debug > 4:
                ...
                if 'StartTime' in line:
                    return False
                # CSV as Argus's ra tool generates it
                # 0:starttime, 1:dur, 2:proto, 3:saddr, 4:sport, 5:dir, 6:daddr: 7:dport, 8:state, 9:stos,  10:dtos, 11:pkts, 12:bytes
                nline = ','.join(line.strip().split(','))
                column_values['starttime'] = datetime.strptime(nline.split(',')[0], '%Y/%m/%d %H:%M:%S.%f') # The time format of CSV is slightly different
                column_values['dur'] = nline.split(',')[1]
                column_values['proto'] = nline.split(',')[2]
                column_values['saddr'] = nline.split(',')[3]
                column_values['sport'] = nline.split(',')[4]
                column_values['dir'] = nline.split(',')[5]
                column_values['daddr'] = nline.split(',')[6]
                column_values['dport'] = nline.split(',')[7]
                column_values['state'] = nline.split(',')[8]
                column_values['pkts'] = nline.split(',')[11]
                column_values['bytes'] = nline.split(',')[12]
            return column_values
        except Exception as inst:
            print '\tProblem in get_columns()'
            ...
            while True:
                if not self.queue.empty():
                    ...
                    if 'stop' != line:
                        ...
                        column_values = self.get_columns(line)
                        if not column_values:
                            continue
                        try:
                            # check if the Ip is not in the whitelist
                            if not column_values['saddr'] in self.ip_whitelist:
                                ...
                                if flowtime < self.slot_starttime:
                                    if args.debug > 10:
                                        print('Flow lost because it belongs to a previous TW: Start:{}, Dur:{:.10f},{},{},{},{},{},{},{},{},{},{},{},{},{}'.format(column_values['starttime'].strftime("%Y-%m-%dT%H:%M:%S.%f"),column_values['dur'],column_values['proto'],column_values['saddr'],column_values['sport'],'<->',column_values['daddr'],column_values['dport'],column_values['state'],0,0,column_values['pkts'],column_values['bytes'],column_values['sbytes'],''))
                                elif flowtime >= self.slot_starttime and flowtime < self.slot_endtime:
                                    if args.debug > 10:
                                        print('Flow in this TW. Start:{}, Dur:{:.10f},{},{},{},{},{},{},{},{},{},{},{},{},{}'.format(column_values['starttime'].strftime("%Y-%m-%dT%H:%M:%S.%f"),column_values['dur'],column_values['proto'],column_values['saddr'],column_values['sport'],'<->',column_values['daddr'],column_values['dport'],column_values['state'],0,0,column_values['pkts'],column_values['bytes'],column_values['sbytes'],''))
                                    # Inside the slot
                                    ...
                                    if args.debug > 10:
                                        print('Flow that changed TW. Start:{}, Dur:{:.10f},{},{},{},{},{},{},{},{},{},{},{},{},{}'.format(column_values['starttime'].strftime("%Y-%m-%dT%H:%M:%S.%f"),column_values['dur'],column_values['proto'],column_values['saddr'],column_values['sport'],'<->',column_values['daddr'],column_values['dport'],column_values['state'],0,0,column_values['pkts'],column_values['bytes'],column_values['sbytes'],''))
                                ...
                                if self.debug:
                                    print blue("Skipping flow with whitelisted ip: {}".format(column_values['saddr']))
